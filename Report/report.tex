...
将两块卷积件各自置于频谱面，在像面可观测到二维点阵的衍射图样，如图\ref{fig:convolution_base_1}至\ref{fig:convolution_base_2}所示；在两块卷积件密接同向放置时，如图\ref{fig:convolution_baseline}所示，能够观察到卷积件2对应的较为细密的衍射图样，以卷积件1衍射图样对应的空间周期以及方向进行排布，并可估计卷积件1, 2光栅常数$d_{1,2}$有以下关系：
...
为验证以上的第2点观察，进一步完善实验，将两块卷积件同步转动至相同角度，并记录相应的衍射图样，如图\ref{fig:convolution_rot_both_00deg}至图\ref{fig:convolution_rot_both_60deg}所示。可以观察到：整体衍射图样与未旋转任一卷积件时情形相似，仅发生了整体的旋转。这就验证了之前归纳的结论。
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{Data/003-pattern/differential.png}
    \caption{光学微分实验结果图}
...
在$\pm 1$级之外，本实验中还可在$\pm 2$级处观察到相应的光学微分像，其特征与$\pm 1$级相似，但整体亮度与对比度均有所降低。
...
使用白光入射提供的物片，获得的频谱面信息如图\ref{fig:theta_modulation_spectrum}所示。通过遮挡比对，可分析出各个空间频率方向所对应的成像区域，并且制作滤波器，选择适当颜色光进行上色。这里选择将城楼门、天空、草地分别处理为红色、蓝色、绿色，所得到的像面图像如图\ref{fig:theta_modulation_image}所示。上色效果较好，验证了$\theta$调制的可行性。
